            ...
            me.executor = executor = new Promise((resolve, reject) => {
                Exec(COMMAND, (error, data) => {
                    ...
                    if (error) {
                        ...
                        console.log('---------- local networking input\n', data)
                        reject(new Error(`${NO_BUENO}:To see the issue please verify the output of ${COMMAND} and see if it includes any valid mac addresses`));
                    }
                    else {
                        resolve(addresses);
                    }
